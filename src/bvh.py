...
# Node
class Node:
    """ A node that structures a 3D model"""

    """ Attributes:
            name : The name of the node
            root : A boolean that is true when the node is the root or False otherwise
            offset : A 3-dimensional vector that saves the offset of the node from its parent
            channels : A 3-dimensional vector that manages the properties of the node
            children : List of its children nodes
    """ 
...
class BVHReader:
    ...
    def __init__(self, filename):
        ...
        # Dictionary for accesing nodes by name
        self.nodeByName = {}
    ...
    def setNodeByName( self , node ):
        """ This recursive functionn will generate a dictionary for accesing nodes by name """
        self.nodeByName[ node.name ] = node
        for child in node.children :
            self.setNodeByName( child )
